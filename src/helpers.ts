...
export const formatTxns = (txns: string[][], account: ObjectId): ITransaction[] => {
  ...
  for(const txn of txns){
    ...
    transactions.push({ledgerType, clearedDate: new Date(date), description,txnType,
      amount: new Decimal128(amountValue), currency, beneficiary, sender, account, createdAt: new Date()
    });
  }
